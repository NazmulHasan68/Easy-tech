...
const TOP_SERVICES: Service[] = [
  { id: "web",     link: "/services/web",      label: "Web Development",   desc: "Next.js & React — fast, scalable, pixel-perfect.",        icon: Globe,       color: "#5e4cdc" },
  { id: "app",     link: "/services/app",      label: "App Development",   desc: "Cross-platform iOS & Android apps users love.",           icon: Smartphone,  color: "#0284c7" },
  { id: "digital", link: "/services/marketing",label: "Digital Marketing", desc: "SEO, social & paid ads that convert clicks to clients.",  icon: TrendingUp,  color: "#16a34a" },
];

const BOTTOM_SERVICES: Service[] = [
  { id: "erp",         link: "/services/erp",                  label: "ERP Solutions", desc: "HR, inventory & finance unified in one dashboard.", icon: Settings2,    color: "#d97706" },
  { id: "media",       link: "/services/media-buying",         label: "Media Buying",  desc: "Meta, Google & TikTok ads for maximum ROI.",        icon: ShoppingCart, color: "#e5484d" },
  { id: "consultancy", link: "/services/business-consultancy", label: "Consultancy",   desc: "Strategy, growth plans & business transformation.", icon: Megaphone,    color: "#7c3aed" },
];

/* ── Card (extracted outside parent — no re-render issues) ── */
function ServiceCard({
  ...
  return (
    <Link
      ...
      className="relative flex-1 min-w-0 rounded-2xl p-4 md:p-5 will-change-transform"
      style={{
...
export default function HomeServicesHub() {
  ...
  const botRefs    = useRef<(HTMLAnchorElement | null)[]>([]);

  const onEnter = (el: HTMLAnchorElement | null, color: string) => {
    if (!el) return;
  ...
  const onLeave = (el: HTMLAnchorElement | null) => {
    if (!el) return;
    ...
    const ctx = gsap.context(() => {
      ...
      if (svgRef.current) {
        ...
        lines.forEach((line, i) => {
          ...
          gsap.to(line, { strokeDashoffset: -24, duration: 1.2 + i * 0.08, ease: "none", repeat: -1 });
        });
      }
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-5 lg:px-10">
        ...
        <div className="text-center mb-14">
          ...
          <p ref={paraRef} className="mt-3 text-sm md:text-base max-w-xl mx-auto will-change-transform"
          ...
          <div className="flex gap-3 md:gap-4 w-full">
            {TOP_SERVICES.map((svc, i) => (
              <ServiceCard
                key={svc.id}
                svc={svc}
                elRef={(el) => { topRefs.current[i] = el; }}
                onMouseEnter={() => onEnter(topRefs.current[i], svc.color)}
                onMouseLeave={() => onLeave(topRefs.current[i])}
              />
            ))}
          </div>
          ...
          <div className="flex gap-3 md:gap-4 w-full">
            {BOTTOM_SERVICES.map((svc, i) => (
              <ServiceCard
                key={svc.id}
                svc={svc}
                elRef={(el) => { botRefs.current[i] = el; }}
                onMouseEnter={() => onEnter(botRefs.current[i], svc.color)}
                onMouseLeave={() => onLeave(botRefs.current[i])}
              />
            ))}
          </div>
