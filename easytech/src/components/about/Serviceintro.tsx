...
export interface ServiceIntroProps {
  /** Small label above title e.g. "Who We Are" */
  label: string;
  /** Title words — last word becomes gradient */ 
  titleWords: string[];
  /** Paragraph description */
  description: string;
  /** Second optional paragraph */
  description2?: string;
  /** Feature checklist */
  features: string[];
  ...
  /** Image alt text */
  imageAlt?: string;
  /** Badge top text e.g. "5+ Years Experience" */
  badgeTop: string;
  /** Badge bottom text e.g. "Web Excellence" */
  badgeBottom: string;
}
...
export default function ServiceIntro({
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const paras = [paraRef.current, para2Ref.current].filter(Boolean);
      gsap.from(paras, {
  ...
  const lastIndex = titleWords.length - 1;
        ...
        <div className="grid grid-cols-1 gap-12 lg:gap-20 items-center">
          ...
          <div ref={imgRef} className="relative will-change-transform">
            ...
            >
              ...
              <h4 className="text-base font-extrabold mt-0.5" style={{ color: "var(--foreground)", fontFamily: "var(--font-sans)" }}>
                {badgeBottom}
              </h4>
            </div>
          </div>
          ...
          <div>
            ...
            >
              {titleWords.map((word, i) => (
                <span
                  key={i}
                  className={`word inline-block will-change-transform${i === lastIndex ? " gradient-text" : ""}`}
                >
                  {word}&nbsp;
                </span>
              ))}
            ...
            <div ref={featuresRef} className="grid sm:grid-cols-2 gap-4">
              {features.map((item, i) => (
                <div key={i} className="flex items-start gap-3 will-change-transform">
                  <CheckCircle size={19} strokeWidth={2} style={{ color: "var(--primary)", flexShrink: 0, marginTop: "2px" }} />
